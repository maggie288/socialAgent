...
5. **设置启动命令（重要）**  
   - 在 **Settings** 里找 **Deploy** → **Custom Start Command** 或 **Start Command**。  
   - **若曾填过** `gunicorn ... $PORT ...` 并报错 `'$PORT' is not a valid port number`：请**清空**该框，或改为 **`sh run.sh`**，让项目用自带的 `run.sh` 读取端口。  
   - 留空时项目会用 Procfile / nixpacks.toml 里的 `sh run.sh` 启动。
